...
class SocialBarIcon extends Component {
    ...
    if (url) {
      return (
        <a href={url} target="_blank" className="column">
          <img src={logo_available} alt={label} className="image" />
        </a>
      );
    } else {
      return (
        <a className="column">
          <img src={logo_unavailable} alt={label} className="image" />
        </a>
      );
    }
  }
  ...
  render() {
    return <div className="column is-paddingless">{this.renderIcon()}</div>;
  }
}
